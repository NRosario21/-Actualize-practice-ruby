require "sqlite3"   # This is loading/ executing code from another file
require "tty-table"  # This is loading/ executing code from another file

class Employee
  @@db = SQLite3::Database.open "employee.db"  # @@ indicates its a class variable / db this is the name of the class variable
  @@db.execute "CREATE TABLE IF NOT EXISTS employees(id INTEGER PRIMARY KEY, first_name TEXT, last_name TEXT, salary INTEGER, active INTEGER)" # the execute method is used to send SQL commands to the database

  attr_reader :id, :first_name, :last_name, :active, :salary
  attr_writer :active

  def initialize(input_options)
    @id = input_options[:id]              # @ indicates instance (object) variable
